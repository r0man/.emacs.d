    ...
    (add-to-list 'ellama-providers
                 (cons "OpenAI GPT-4"
                       (make-llm-openai
                        ...
                        :chat-model "gpt-4-turbo-preview"
                        :embedding-model "text-embedding-ada-002")))
    (setq ellama-provider (alist-get "OpenAI GTP-3" ellama-providers nil nil #'string=)))
...
#+begin_src emacs-lisp
  (use-package nu-llm
    ...
    (when (fboundp 'nu-llm-make-openai)
      (add-to-list 'ellama-providers (cons "Nu OpenAI GPT-3" (nu-llm-make-openai :chat-model "gpt-3.5-turbo-0613"))))
    (when (fboundp 'nu-llm-make-openai)
      (add-to-list 'ellama-providers (cons "Nu OpenAI GPT-4" (nu-llm-make-openai :chat-model "gpt-4-turbo-preview")))
      (setq ellama-provider (alist-get "Nu OpenAI GPT-4" ellama-providers nil nil #'string=)))
    (when (fboundp 'nu-llm-make-api)
      (add-to-list 'ellama-providers (cons "Nu LLM" (nu-llm-make-api)))))
#+end_src
** Elixir
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (use-package cider
    ...
    (cider-add-to-alist 'cider-jack-in-dependencies "refactor-nrepl/refactor-nrepl" "3.12.0")
...
#+begin_src emacs-lisp
  (use-package compile
    :commands (compile)
    :hooks (compilation-filter-hook . colorize-compilation-buffer)
    :custom
