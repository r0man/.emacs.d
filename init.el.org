...
#+begin_src emacs-lisp
  (use-package eca
    ;; :load-path ("~/workspace/workspace/eca-emacs")
    :custom
    (eca-custom-command '("eca" "server" "--log-level" "debug")))
#+end_src
** Eldoc
    ...
    (add-to-list 'ellama-providers
                 (cons "OpenAI o3"
                       (make-llm-openai
                        :key (auth-source-pick-first-password :host "openai.com" :user "ellama")
                        :chat-model "o3")))
    (setq ellama-provider (alist-get "OpenAI GPT-4.1" ellama-providers nil nil #'string=)))
