...
(add-hook
 ...
 (lambda ()
   ...
   (let ((mode-map emacs-lisp-mode-map))
     (define-key mode-map (kbd "C-c e E") 'elint-current-buffer)
     (define-key mode-map (kbd "C-c e c") 'cancel-debug-on-entry)
     (define-key mode-map (kbd "C-c e d") 'debug-on-entry)
     (define-key mode-map (kbd "C-c e e") 'toggle-debug-on-error)
     (define-key mode-map (kbd "C-c e f") 'emacs-lisp-byte-compile-and-load)
     (define-key mode-map (kbd "C-c e l") 'find-library)
     (define-key mode-map (kbd "C-c e r") 'eval-region)
     (define-key mode-map (kbd "C-c e b") 'eval-buffer))
